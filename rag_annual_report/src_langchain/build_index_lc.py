"""
向量索引构建脚本（LangChain 版）- AI技术面试知识库
...
VECTORSTORE_DIR = Path("E:/vectorstore/faiss_lc")          # 修改为纯英文路径
...
def load_documents():
    ...
    从 manifest.json 读取论文元数据（标题、作者、年份、主题），
    而不是从文件名解析股票代码。
    """
    import json
    ...
    if not pdf_files:
        raise FileNotFoundError(f"data/raw_pdf/ 目录下没有 PDF 文件，请先将论文 PDF 放入该目录")

    all_docs = []
    
    filename_to_meta = {item["filename"]: item for item in manifest}
    ...
    for pdf_path in sorted(pdf_files):
        ...
        meta = filename_to_meta.get(pdf_path.name, {})
        for doc in docs:
            doc.metadata["doc_type"] = meta.get("doc_type", "paper")
            doc.metadata["title"]    = meta.get("title", "")
            doc.metadata["author"]   = meta.get("author", "")
            doc.metadata["year"]     = meta.get("year", "")
            doc.metadata["topic"]    = meta.get("topic", "")
